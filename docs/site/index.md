...
**Capability profiles** control shell access, file write permissions, and database mutation behavior per agent. A `readonly` agent can safely inspect production systems. A `developer` agent can edit files and run arbitrary commands. An `eval` agent has unrestricted access for sandboxed benchmark execution.
...
**Multi-agent coordination** lets agents delegate focused subtasks to child agents and lets operators coordinate and control running agents through the **command center TUI**.

**Built-in observability** tracks sessions, turns, token usage, and tool execution metrics to a SQLite backend or OTLP endpoint, with the **command center TUI** as the primary live inspection surface.
...
| [Observability](observability/) | Telemetry, command center, and legacy dashboard/monitor |
